...
- **The relationship vocabulary is reference data, not an enum** ([ADR-0040](docs/adr/adr-0040-relationship-vocabulary-reference-data.md)). `relationship_type` is an FK into `relationship_types`; adding a type is an `INSERT`, never a migration or a code change. Don't reintroduce a hard-coded list — not in SQL, not as a Zod enum, not as a UI array. Behaviour and presentation come from the row: `is_symmetric`/`inverse_key` drive reciprocal creation, `direction_verb`/`symmetric_noun` the card text, `category_key`/`sort_order` the picker grouping. `(relationship_type, relationship_role)` is a composite FK whose `MATCH SIMPLE` NULL-skip is what makes a sub-role optional — see `docs/system-design.md` §3.3.1.
