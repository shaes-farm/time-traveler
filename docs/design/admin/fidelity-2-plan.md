...
| **[I](#batch-i--media-library--picker)**           | [#49](https://github.com/shaes-farm/time-traveler/issues/49) per-entity core (upload/external/attach — done on `feat/49-media-management`); cross-entity library + reusable picker (screen 17) decomposed into [#291](https://github.com/shaes-farm/time-traveler/issues/291)–[#295](https://github.com/shaes-farm/time-traveler/issues/295)                                                                                     | in progress |
...
| **[K](#batch-k--timeline-surfaces--fractal-tree)** | [#42](https://github.com/shaes-farm/time-traveler/issues/42), [#43](https://github.com/shaes-farm/time-traveler/issues/43), [#44](https://github.com/shaes-farm/time-traveler/issues/44), [#47](https://github.com/shaes-farm/time-traveler/issues/47) (timeline list/editor/detail + event detail + bespoke `Tree`) — all closed; fractal drill-down deferred to [#296](https://github.com/shaes-farm/time-traveler/issues/296) | done        |
| **[L](#batch-l--publish-workflow--collaborators)** | [#48](https://github.com/shaes-farm/time-traveler/issues/48) (publish/unpublish), [#50](https://github.com/shaes-farm/time-traveler/issues/50) (collaborator management)                                                                                                                                                                                                                                                         | done        |
...
> **Issue tracking.** Batch I splits into two halves. The **per-entity core** ([#49](https://github.com/shaes-farm/time-traveler/issues/49) — upload/external/attach/detach/order on event/character/timeline detail, screen 15) is implemented on `feat/49-media-management`. The **cross-entity library + reusable picker** (screen 17), which #49 lists under Non-Goals as a deferred follow-up, is decomposed into five issues against Phase 5 (the wireframe promotes screen 17 to Phase 5):
>
> - [#291](https://github.com/shaes-farm/time-traveler/issues/291) — `media-service` library query layer (faceted search/filter/pagination + cross-entity attachment map + orphan detection)
> - [#292](https://github.com/shaes-farm/time-traveler/issues/292) — `MediaPicker` shared primitive (browse + pick) with `MediaGrid` / `MediaCard` / `MediaFilterRail` + library hooks — **build first**
> - [#293](https://github.com/shaes-farm/time-traveler/issues/293) — `MediaDetailDrawer` (edit `media` row + attachment list + blast-radius delete)
> - [#294](https://github.com/shaes-farm/time-traveler/issues/294) — Media Library route (`/media`, screen-17 browser) + upload entry points + orphan cleanup
> - [#295](https://github.com/shaes-farm/time-traveler/issues/295) — Attach-dialog **Existing** tab + character/event editor picker wiring
...
- **Source facet + kind badge (unblocked).** [#179](https://github.com/shaes-farm/time-traveler/issues/179) (nullable `storage_path` + `source` discriminator) **landed and is closed** — the `storage_path IS NULL` stopgap and `provisional` facet marker are no longer needed; the **Source** facet and kind badge read the real `source` column.
...
- **`Switch` primitive** (`switch.tsx`) — the A.3-deferred toggle; dependency-free (`<button role="switch">`, no new Radix peer). Consumed by editor-level draft/publish toggles where those toggles exist.
...
### Batch I — Media (status)

Specified in [Batch I](#batch-i--media-library--picker) above. The **per-entity core** ([#49](https://github.com/shaes-farm/time-traveler/issues/49), the third Phase-4 surface) is implemented on `feat/49-media-management`. The **cross-entity library + picker** (screen 17) is decomposed into [#291](https://github.com/shaes-farm/time-traveler/issues/291)–[#295](https://github.com/shaes-farm/time-traveler/issues/295) against Phase 5 and not yet implemented. The Source facet + kind badge are **unblocked** — [#179](https://github.com/shaes-farm/time-traveler/issues/179) landed, so they read the real `source` discriminator (no stopgap).
