...
- **Era-palette reconciliation** — the shipped Batch E era tokens (hue-spread for colorblind separation) are the source of truth; this batch records the divergence from PRD §7.2.2 and **files the PRD-fix issue** so the PRD palette is updated to match (treat like the [#127](https://github.com/shaes-farm/time-traveler/issues/127) reconciliation). No token value change — the code already ships the finalized hues.
