...
- **`Switch` primitive** (`switch.tsx`) — the A.3-deferred toggle; dependency-free (`<button role="switch">`, no new Radix peer). Consumed by the timeline editor's inline publish toggle.
