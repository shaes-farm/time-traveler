...
7. **Profile media is single-image in this view.** Multiple images live in the Media tab on the detail view. The editor sets the primary; additional media is a detail-view concern. The picker behind the image slot is the shared Attach dialog — its **Existing** tab lets you reuse a `media` row already in the library rather than re-uploading (see [15-media-management.md](15-media-management.md) and [17-media-library.md](17-media-library.md)).
