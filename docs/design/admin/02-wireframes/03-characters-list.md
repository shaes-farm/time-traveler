...
7. **Status column** shows three states (per PRD §7.11.5; #127 reconciliation): `✓ Published`, `─ draft` for Draft, and `⇄ shared` for characters reachable to the user via `timeline_collaborators` (the character belongs to another user but is visible because the current user collaborates on a timeline that features events including this character). The Shared state is a permission-context marker, not a content-state marker — the underlying character may itself be Published or Draft from its owner's perspective. Specific colors (green / gray / blue per PRD §7.11.5) and icons applied at fidelity-2 visual design.
8. **Pagination is offset-based for now**, with sort selector on the right. The sort selector offers Name / Created / Last updated / Birth date; "Birth date" orders by the `characters.sort_order_years` generated column (migration 00025, #326), with undated characters (e.g. divine/mythological with neither birth nor death) sorting last. Cursor pagination on `sort_order_years` (system-design §8.2) is now feasible but remains deferred.
