...
- **New database tables need explicit Data API GRANTs** (Supabase June-2026 change; [ADR-0034](../docs/adr/adr-0034-api-role-table-grants.md)). Tables in `public` are no longer auto-exposed — PostgREST returns `42501` until a role is granted on the table. GRANT controls table access; RLS controls which rows — **both** are required. Any migration that `CREATE`s a table (including junction tables) must, in the same file, enable RLS, add policies, **and** grant least-privilege: `GRANT SELECT ON public.<table> TO anon;` plus `GRANT SELECT, INSERT, UPDATE, DELETE ON public.<table> TO authenticated, service_role;` (`anon` is read-only — the reader app is anonymous). Do **not** re-enable auto-exposure via `ALTER DEFAULT PRIVILEGES` (a temporary aid Supabase removes 2026-10-30). See `supabase/migrations/00023_api_role_table_grants.sql` for the canonical snippet, and run `pnpm db:reset && pnpm db:test` after schema changes to catch missing grants.
